...
$stmt = $con->prepare("select CASE WHEN doctor.first_name IS not NULL THEN doctor.first_name ELSE user.first_name END as first_name , 
CASE WHEN doctor.last_name IS not NULL THEN doctor.last_name ELSE user.last_name END as last_name ,
 ...
 CASE WHEN doctor.first_name IS not NULL THEN 'doctor' ELSE 'user' END as commenter , comment_log.id , 
 comment_log.date_share , comment_log.content from comment_log left JOIN user on comment_log.user = user.id left join doctor on doctor.id = comment_log.doctor
where user.first_name is not null or doctor.first_name is not null");
$stmt->execute();
$comment_log = $stmt->fetchAll(PDO::FETCH_ASSOC);
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
                    ...
                    <ul class="navbar-nav ml-auto">
                        <div style="float:right ; margin-right:10px ;margin-top:20px" class="nav-menu">
                            <ul>
                                <li style="float:left;list-style-type: none;"><a href="commentUser.php?lang=ar">العربية</a></li>
                                <li style="float:left;list-style-type: none;">|</li>
                                <li style="float:left;list-style-type: none;"><a href="commentUser.php?lang=en">English</a></li>
                            </ul>
                        </div>
                                    ...
                                    ?>
                                        <tbody id="row-<?php echo $id ?>">
                                            <tr>
                                                ...
                                                <th><a href="../<?php echo $row['commenter']?>profile.php?email=<?php echo $row['email']; ?>"><?php echo $row['first_name'] . ' ' . $row['last_name']; ?></a></th>
