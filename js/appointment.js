                                    ...
                                    $("#appointment_list").append(
                                        ...
                                        <tr class='myList'>
                                            ...
                                            <td class="text-center align-middle">
                                              <button class="btn btn-danger btn-sm " onclick="delete_appointment(${element.id})"><i class="fa fa-trash"></i></button>
                                          </td>
                                        </tr>
                                        `
                                    );
...
// on change deptartment to select doctor
$(document).ready(function () {
    $('#dept').change(function () {
        ...
        $.ajax({
            ...
            if (lang == 'en') {
                ...
                $('#doctor').append(`<option value='0'>اختر طبيب</option>`);
            }
            data = JSON.parse(data);
            data.forEach(function (data) {
                $('#doctor').append("<option value= " + data.id + "> " + data.first_name + " " + data.last_name + "</option>")
            })
        })
    });
});

// load table doctor on change doctor value
$(document).ready(function () {
    $('#doctor').change(function () {
        ...
        $.ajax({
            ...
            data = JSON.parse(data);
            $('.list').empty();
            $("#time").val(`${data[0].sTimeDoctor.substr(0, 5)}-${data[0].eTimeDoctor.substr(0, 5)}`);
            if (data.length !== 0) {
                for (let i = 0; i < data.length; i++) {
                    const element = data[i];
                    $("#appointment_list_doctor").append(
